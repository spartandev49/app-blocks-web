function appBlocksDesignRuntime() {
  "use strict";

  const doc = document;
  const root = doc.documentElement;
  const body = doc.body;
  const reduced = Boolean(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) || root.dataset.motion === "off";
  const qa = (selector, scope = doc) => Array.from(scope.querySelectorAll(selector));
  ...
  function optionFromClass(element, name) {
    const prefix = `abx-opt-${name}-`;
    const match = Array.from(element.classList).find((token) => token.startsWith(prefix));
    return match ? match.slice(prefix.length) : "";
  }

  function firstFocusable(scope) {
    return scope.querySelector(focusableSelector);
  }
  ...
  function initMotion() {
    ...
    requestAnimationFrame(() => {
      items.filter((item) => item.getBoundingClientRect().top < innerHeight).forEach((item) => item.classList.add("is-abx-visible"));
    });
  }

  function initCarousels() {
    qa(".abx-carousel").forEach((carousel, carouselIndex) => {
      const explicit = qa(".abx-slide", carousel);
      const slides = explicit.length ? explicit : qa(":scope > .ab-panel,:scope > .ab-card,:scope > div > .ab-panel,:scope > div > .ab-card", carousel);
      if (slides.length < 2) return;
      let index = Math.max(0, slides.findIndex((slide) => slide.hasAttribute("data-selected") || slide.classList.contains("is-selected")));
      let timer = 0;
      carousel.id ||= `abx-carousel-${carouselIndex + 1}`;
      carousel.setAttribute("role", "region");
      carousel.setAttribute("aria-roledescription", "carousel");
      carousel.setAttribute("aria-label", carousel.getAttribute("aria-label") || "Content carousel");
      ...
      const show = (nextIndex, announce = true) => {
        ...
        slides.forEach((slide, slideIndex) => {
          ...
          qa(focusableSelector, slide).forEach((control) => {
            if (!active) {
      ...
      const start = () => {
        if (!reduced && interval && !timer) timer = window.setInterval(() => show(index + 1, false), interval);
      };
  ...
  function initDrawers() {
    ...
    doc.addEventListener("keydown", (event) => {
      ...
      const focusable = qa(focusableSelector, active);
  ...
  function initDisclosureMenus() {
    ...
    doc.addEventListener("click", (event) => {
      qa(".abx-dropdown.is-abx-open,.abx-popover.is-abx-open").forEach((container) => {
  ...
  function initCommandPalette() {
    ...
    doc.addEventListener("keydown", (event) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
        event.preventDefault();
        if (palette.classList.contains("is-abx-open")) close(); else open();
    ...
    qa(".abx-counter,.abx-metric-card,.abx-stat-card").forEach((container) => {
      ...
      const run = () => {
        ...
        const frame = (now) => {
          ...
          const rendered = (value * eased).toLocaleString(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
          target.textContent = `${prefix}${rendered}${suffix}`;
  ...
  function initFields() {
    ...
    qa(".abx-file-drop").forEach((zone) => {
      ...
      ["dragenter", "dragover"].forEach((name) => zone.addEventListener(name, (event) => {
        event.preventDefault();
        zone.classList.add("is-abx-dragging");
      }));
      ["dragleave", "drop"].forEach((name) => zone.addEventListener(name, (event) => {
        event.preventDefault();
        zone.classList.remove("is-abx-dragging");
      }));
      zone.addEventListener("drop", (event) => {
        if (!event.dataTransfer?.files?.length) return;
        try { input.files = event.dataTransfer.files; } catch { /* Native file selection remains available. */ }
  ...
  function initScrollEffects() {
    ...
    const update = () => {
      ...
      const percent = Math.max(0, Math.min(100, scrollY / maximum * 100));
      progress.forEach((item) => item.style.setProperty("--abx-scroll", `${percent}%`));
      if (!reduced) {
        parallax.forEach((item) => {
          const rect = item.getBoundingClientRect();
          const offset = Math.max(-28, Math.min(28, (rect.top + rect.height / 2 - innerHeight / 2) * -0.035));
  ...
  function initPointerEffects() {
    ...
    qa(".ab-button[class*='abx-button']").forEach((button) => {
  ...
  function initActions() {
    qa("button,[role='button'],a").forEach((control) => {
      const message = actionValue(control).match(/^toast[:=-](.+)$/)?.[1];
      if (!message) return;
...
export const V2_RUNTIME_JS = `;(${appBlocksDesignRuntime.toString()})();`;
