...
export const CATALOG = Object.freeze([...byName.values()]);
...
function virtualManifest(name) {
  for (const family of VIRTUAL_FAMILIES) {
    ...
    return {
      name,
      category: ["hero", "cta", "pricing", "testimonials", "feature"].includes(family.canonical) ? "marketing" : ["form", "table", "dialog", "chart", "metric", "app-shell"].includes(family.canonical) ? "application" : "layout",
...
function stabilizeManifest(result, originalSource, design) {
  ...
  result.manifest.files = [...result.files.keys()].sort();
